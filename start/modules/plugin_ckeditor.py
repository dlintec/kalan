...
class CKEditor(object):
    ...
    def widget(self, field, value, **attributes):
        ...
        attributes['_class'] = 'text plugin_ckeditor widget_contenidoHTML'
        textarea = TEXTAREA(**attributes)
        textarea['_contenteditable']="true"
        #textarea.append(XML(value))
    ...
    def widgetant(self, field, value, **attributes):
        """
        To be used with db.table.field.widget to set CKEditor as the desired
        widget for the field. Simply set
        db.table.field.widget = ckeditor.widget to use the CKEditor widget.
    ...
    def handle_upload(self):
        ...
        if upload != None:
            ...
            raise HTTP(401, T('Missing required upload.'))
    ...
    def load(self, selector=None, use_caching=True):
        ...
        ckeditor_js = URL('static', 'dit/ckeditor/ckeditor.js')
        jquery_js = URL('static', 'dit/ckeditor/adapters/jquery.js')

        contents_css = "['%s']" % URL('static', 'dit/ckeditor/contents.css')
        #**********************dlintec
        contents_css ="[";
        contents_css=contents_css+"'"+URL('static', 'dit/ckeditor/contents.css')+"',"
        contents_css=contents_css+"'"+URL('static', 'css/bootstrap.min.css')+"',"
        contents_css=contents_css+"'"+URL('static', 'css/bootstrap-theme.min.css')+"',"
        contents_css=contents_css+"'"+URL('static', 'css/web2py-bootstrap3.css')+"',"
        contents_css=contents_css+"'"+URL('static', 'dit/css/dit_custom.css')+"',"
        contents_css=contents_css+"'"+URL('static', 'dit/css/dit_3d.css')+"',"
        ...
        cadToolbar="""
                        toolbar :
                        [
                            { name: 'items', items : [ 'Maximize','-', 'Bold','Italic','Underline','TextColor','BGColor',
								 'NumberedList','BulletedList',"RemoveFormat",'SpecialChar','Link','Unlink',
 								 'Outdent','Indent','Blockquote','JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock',
                                 'Image','CreatePlaceholder','Table','HorizontalRule','PageBreak','Iframe','ShowBlocks',
                                 '-','Undo','Redo','-','Cut','Copy','Paste','PasteText','PasteFromWord','DocProps','Preview','Print','Document','Source',
        ...
        cadXML=            """
            ...
            <script type="text/javascript">
                CKEDITOR.on( 'dialogDefinition', function( evt ) {
                    var dialog = evt.data;
                    console.log('DIALOG DEFINITION:',dialog);
                    if ( dialog.name == 'image' ) {
                        var def = evt.data.definition;
                        dialog.definition.minWidth=200;
                        dialog.definition.width=200;
                    }
                    /*
                    if ( dialog.name == 'link' ) {
                ...
                function ckeditor_config() {
                        ...
                        [
                            { name: 'items', items : [ 'Maximize','-', 'Bold','Italic','Underline','TextColor','BGColor',
								 'NumberedList','BulletedList',"RemoveFormat",'SpecialChar','Link','Unlink',
 								 'Outdent','Indent','Blockquote','JustifyLeft','JustifyCenter','JustifyRight','JustifyBlock',
                                 'Image','CreatePlaceholder','Table','HorizontalRule','PageBreak','Iframe','ShowBlocks',
                                 '-','Undo','Redo','-','Cut','Copy','Paste','PasteText','PasteFromWord','DocProps','Preview','Print','Document',
                                 'Format','Font','FontSize',
                                                     ] },
                        ];
                    /*var enMobile=md.mobile();
                    ...
                    if (enMobile!=null){
                        ...
                        console.log('ckeditor_config:',objConfig)
                    return objConfig
                }
                %(immediate)s
            </script>
