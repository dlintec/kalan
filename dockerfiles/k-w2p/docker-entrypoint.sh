...
if [[  ( "$par1" = 'initadmin') && ( -n "$par2" ) ]]; then
  ...
  chown -R kcontainer:kcontainer /var/kalan-container/web2py/applications/
  exec /usr/bin/python /var/kalan-container/web2py/web2py.py --nogui -i 0.0.0.0 -p 8443 -a "$par2" -k $container_sslfolder/self_signed.key -c $container_sslfolder/self_signed.cert
